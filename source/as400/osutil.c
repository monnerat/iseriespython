/*
 osutil.c - native operating system services  (used by hg)
...
#ifdef __ILEC400__
#include <as400misc.h>
static const S_IFMT = 61440;
static const S_IFDIR = 16384;
static char e_buf[2048];
#endif
...
#else
#ifdef __ILEC400__
	dir = opendir(utfToStr(path, e_buf));
#else
	dir = opendir(path);
#endif
#endif
...
#ifdef __ILEC400__
#pragma convert(0)
		strToUtf(ent->d_name, e_buf, 2047);
#endif
		...
		/* quit early? */
#ifdef __ILEC400__
		if (skip && kind == S_IFDIR && !strcmp(e_buf, skip)) {
#else
...
#ifdef __ILEC400__
			elem = Py_BuildValue("siN", e_buf, kind, stat);
#else
			elem = Py_BuildValue("siN", ent->d_name, kind, stat);
#endif
		} else
#ifdef __ILEC400__
			elem = Py_BuildValue("si", e_buf, kind);
#else
