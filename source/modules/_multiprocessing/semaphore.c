...
int
#ifdef __ILEC400__
sem_timedwait_save(sem_t *__ptr128 sem, struct timespec *deadline, PyThreadState *_save)
#else
...
static PyObject *
#ifdef __ILEC400__
newsemlockobject(PyTypeObject *type, SEM_HANDLE handle, int kind, int maxvalue, char *name)
#else
...
#ifdef __ILEC400__
    PyOS_snprintf(buffer2, sizeof(buffer), "/mp%ld-%d-%d", (long)getpid(),
            (int)time(&t), counter++);
    utfToStr(buffer2, buffer);
#else
...
#ifdef __ILEC400__
    result = newsemlockobject(type, handle, kind, maxvalue, buffer);
#else
...
#endif
    if (handle == SEM_FAILED) {
        PyErr_SetString(PyExc_ValueError, "Could not recreate semaphore");
        return NULL;
    }
...
static PyMemberDef semlock_members[] = {
    ...
    {"maxvalue", T_INT, offsetof(SemLockObject, maxvalue), READONLY,
     ""},
#ifdef __ILEC400__
    {"name", T_STRING_INPLACE, offsetof(SemLockObject, name), READONLY,
     ""},
#endif
    {NULL}
};
