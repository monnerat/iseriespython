...
sets(PyObject *v, int i, char* val)
{
  if (val)
#ifdef __ILEC400__
	  PyStructSequence_SET_ITEM(v, i, strLenToUtfPy(val, strlen(val)));
#else
...
#if defined(__VMS) || defined(__ILEC400__)
    SETS(setIndex++, "");
#else
...
pwd_getpwuid(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "I:getpwuid", &uid))
		return NULL;
#ifdef __ILEC400__
	if (getpwuid_r(uid, p, pwdbuff, 1024, &pt) != 0) {;
#else
...
pwd_getpwnam(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "s:getpwnam", &name))
		return NULL;
#ifdef __ILEC400__
	if (getpwnam_r(utfToStr(name, nambuff), p, pwdbuff, 1024, &pt) != 0) {;
#else
