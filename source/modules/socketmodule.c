...
newPySSLI5Env(char *applId, int sessionType, int authType)
{
    ...
    if (applId != NULL && strlen(applId) > 0)
    {
        ...
        PyObject *obj = PyString_FromStringAndSize(NULL, strlen(applId) + 1);
...
newPySSLI5Session(PySSLI5Env *env, PySocketSockObject *sock, int certReq)
{
    PySSLI5Session *self;
    int rc, retcode;
...
static PyObject *
sock_sslInit(PyObject *self, PyObject *args)
{
    PySSLI5Env *env;
    char *applId = NULL;
    int sessionType = 0;
...
PyDoc_STRVAR(sslInit_doc,
"sslInit(application_id, session_type, [auth_type]) -> SSLI5Environment");

static PyObject *
PySSLI5EnvOpen(PySSLI5Env *self, PyObject *args)
{
    PySSLI5Session *session;
    PySocketSockObject *sock;
    int certReq = 1;

	if (!PyArg_ParseTuple(args, "O!|i:open", &sock_type, (PyObject*)&sock,
                &certReq))
		return NULL;

    session = newPySSLI5Session(self, sock, certReq);
    if (session == NULL)
        return NULL;
...
PySSLI5SessionWrite(PySSLI5Session *self, PyObject *args)
{
    ...
    if(PyErr_CheckSignals()) {
        PyErr_SetString(ssli5Error, "Interrupt occured.");
        return NULL;
    }
    if (rc != GSK_OK) {
...
PySSLI5SessionRead(PySSLI5Session *self, PyObject *args)
{
    ...
    if (sockstate == SOCKET_HAS_TIMED_OUT) {
        PyErr_SetString(ssli5Error, "The read operation timed out");
        return NULL;
    } else if (sockstate == SOCKET_HAS_BEEN_CLOSED) {
        PyErr_SetString(ssli5Error, "Underlying socket has been closed.");
        return NULL;
...
{
#if defined(__ILEC400__) && __ILEC400_TGTVRM__ < 600
    struct addrinfo hints, *__ptr128 res = NULL;
#else
...
#ifdef __ILEC400__
#pragma convert(37)
#if __ILEC400_TGTVRM__ > 600
		error = getaddrinfo_ts64(NULL, "0", &hints, &res);
#else
...
#pragma convert(0)
    ...
    ACQUIRE_GETADDRINFO_LOCK
#ifdef __ILEC400__
    utfToStr(name, namebuf); 
#if __ILEC400_TGTVRM__ > 600
    error = getaddrinfo_ts64(namebuf, NULL, &hints, &res);
#else
    error = getaddrinfo(namebuf, NULL, &hints, &res);
#endif
#else
...
#ifdef __ILEC400__
	return PyString_FromString(strToUtf(buf, buf, strlen(buf)));
#else
...
{
#ifdef __ILEC400__
	extern int fclose(FILE * __ptr128);
#else
...
socket_gethostname(PyObject *self, PyObject *unused)
{
    ...
    buf[sizeof buf - 1] = '\0';
#ifdef __ILEC400__
	return strLenToUtfPy(buf, strlen(buf));
#else
	return PyString_FromString(buf);
#endif
}
...
#pragma datamodel(pop)
    ...
    if (h->h_aliases) {
        for (pch = h->h_aliases; *pch != NULL; pch++) {
            int status;
#ifdef __ILEC400__
            tmp = strLenToUtfPy(*pch, strlen(*pch));
#else
...
#ifdef __ILEC400__
    tmp = strLenToUtfPy(h->h_name, strlen(h->h_name));
    rtn_tuple = Py_BuildValue("OOO", tmp, name_list, addr_list);
    Py_DECREF(tmp);
#else
...
#ifdef __ILEC400__
    char namebuf[1025], protobuf[129];
#endif
    struct servent *sp;
...
#ifdef __ILEC400__
    name = utfToStr(name, namebuf);
    if (proto != NULL)
        proto = utfToStr(proto, protobuf);
#endif
    Py_BEGIN_ALLOW_THREADS
...
#ifdef __ILEC400__
        ip_addr = utfToStr(ip_addr, ipbuf);
#endif
...
#ifdef __ILEC400__
	s = inet_ntoa(packed_addr);
	return strLenToUtfPy(s, strlen(s));
#else
...
#ifdef __ILEC400__
        return strLenToUtfPy((char *)retval, strlen(retval));
#else
...
{
#if defined(__ILEC400__) && __ILEC400_TGTVRM__ < 600
    struct addrinfo hints, *__ptr128 res;
    struct addrinfo *__ptr128 res0 = NULL;
#else
    struct addrinfo hints, *res;
...
#ifdef __ILEC400__
   if (hptr != NULL)
       hptr = utfToStr(hptr, hbuf);
   if (pptr != NULL)
       pptr = utfToStr(pptr, pbuf);
#endif
    memset(&hints, 0, sizeof(hints));
...
#ifdef __ILEC400__
#if __ILEC400_TGTVRM__ > 600
	error = getaddrinfo_ts64(hptr, pptr, &hints, &res0);
#else
...
#ifdef __ILEC400__
#if __ILEC400_TGTVRM__ > 600
	error = getaddrinfo_ts64(hostp, pbuf, &hints, &res);
#else
...
#ifdef __ILEC400__
	ret = Py_BuildValue("ss", strToUtf(hbuf, hbuf, strlen(hbuf)), strToUtf(pbuf, pbuf, strlen(pbuf))); 
#else
...
#ifdef __ILEC400__
    {"sslInit",   (PyCFunction)sock_sslInit, METH_VARARGS,
              sslInit_doc},
#endif
...
init_socket(void)
{
    ...
    if (PyType_Ready(&PySSLI5Env_Type))
        return;
    PySSLI5Session_Type.ob_type = &PyType_Type;
    if (PyType_Ready(&PySSLI5Session_Type))
        return;
    d = PyModule_GetDict(m);
    ssli5Error = PyErr_NewException("ssli5.error", NULL, NULL);
    PyDict_SetItemString(d, "sslerror", ssli5Error);
    Py_INCREF(ssli5Error);
    if (PyDict_SetItemString(d, "SSLEnvType",
                 (PyObject *)&PySSLI5Env_Type) != 0)
        return;
    Py_INCREF(&PySSLI5Env_Type);
    if (PyDict_SetItemString(d, "SSLSessionType",
                 (PyObject *)&PySSLI5Session_Type) != 0)
        return;
    Py_INCREF(&PySSLI5Session_Type);
    PyModule_AddIntConstant(m, "GSK_CLIENT_SESSION",
                GSK_CLIENT_SESSION);
    PyModule_AddIntConstant(m, "GSK_SERVER_SESSION",
                GSK_SERVER_SESSION);
    PyModule_AddIntConstant(m, "GSK_SERVER_SESSION_AUTH",
                GSK_SERVER_SESSION_WITH_CL_AUTH);
    PyModule_AddIntConstant(m, "GSK_CLIENT_AUTH_FULL",
                GSK_CLIENT_AUTH_FULL);
    PyModule_AddIntConstant(m, "GSK_CLIENT_AUTH_PASSTHRU",
                GSK_CLIENT_AUTH_PASSTHRU);
    PyModule_AddIntConstant(m, "GSK_CLIENT_AUTH_REQUIRED",
                GSK_OS400_CLIENT_AUTH_REQUIRED);
    PyModule_AddIntConstant(m, "GSK_SERVER_AUTH_FULL",
                GSK_SERVER_AUTH_FULL);
    PyModule_AddIntConstant(m, "GSK_SERVER_AUTH_PASSTHRU",
                GSK_SERVER_AUTH_PASSTHRU);
    PyModule_AddIntConstant(m, "GSK_ERROR_NO_CERTIFICATE",
                GSK_ERROR_NO_CERTIFICATE);
    PyModule_AddIntConstant(m, "GSK_ERROR_BAD_CERTIFICATE",
                GSK_ERROR_BAD_CERTIFICATE);
    PyModule_AddIntConstant(m, "GSK_OK",
                GSK_OK);
#endif
