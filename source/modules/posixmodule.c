...
#define HAVE_SYSTEM 1
#define HAVE_WAIT       1
/* ebcedic buffer, used to temporarily convert to ebcedic */
static char e_buf[1025];
static char e_buf2[1025];
#else			/* all other compilers */
...
#ifdef __ILEC400__
        v = strLenToUtfPy(p+1, strlen(p+1)); 
#elif
...
static PyObject *
#if defined(__ILEC400__)
posix_1str(PyObject *args, char *format, int (* __ptr128 func)(const char* __ptr128))
#else
...
posix_2str(PyObject *args,
	   char *format,
	   int (* __ptr128 func)(const char * __ptr128, const char * __ptr128))
#else
posix_2str(PyObject *args,
           char *format,
...
#endif
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = (*func)(utfToStr(path1, e_buf), utfToStr(path2, e_buf2));
#else
...
#elif __ILEC400__
              int (* __ptr128 statfunc)(const char * __ptr128, STRUCT_STAT * __ptr128),
#else
              int (*statfunc)(const char *, STRUCT_STAT *),
#endif
              char *wformat,
              int (*wstatfunc)(const Py_UNICODE *, STRUCT_STAT *))
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = (*statfunc)(utfToStr(path, e_buf), &st);
#else
...
finish:
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = access(utfToStr(path, e_buf), mode);
#else
...
posix_chmod(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = chmod(utfToStr(path, e_buf), i & 07777);
#else
	res = chmod(path, i);
#endif
    Py_END_ALLOW_THREADS
...
posix_chown(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = chown(utfToStr(path, e_buf), (uid_t) uid, (gid_t) gid);
#else
...
posix_lchown(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = lchown(utfToStr(path, e_buf), (uid_t) uid, (gid_t) gid);
#else
...
#if (defined(__sun) && defined(__SVR4)) || defined(__OpenBSD__) || defined(__ILEC400__)
...
posix_getcwd(PyObject *self, PyObject *noargs)
{
    ...
    if (res == NULL)
        return posix_error();
#ifdef __ILEC400__
	return PyString_FromString(strToUtf(buf, e_buf, 1024));
#else
...
posix_getcwdu(PyObject *self, PyObject *noargs)
{
    ...
    if (res == NULL)
        return posix_error();
#ifdef __ILEC400__
	strToUtf(buf, e_buf, 1024);
	return PyUnicode_Decode(e_buf, strlen(e_buf), Py_FileSystemDefaultEncoding,"strict");
#else
...
#define MAX_PATH    CCHMAXPATH
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
    dirp = opendir(utfToStr(name, e_buf));
#else
    dirp = opendir(name);
#endif
    Py_END_ALLOW_THREADS
...
#ifdef __ILEC400__
#pragma convert(0)
		v = PyString_FromString(strToUtf(ep->d_name, e_buf, 1024));
#else
...
posix_mkdir(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = mkdir(utfToStr(path, e_buf), mode);
#else
...
error:
	...
	if (!PyArg_ParseTuple(args, "etet:rename",
	                      ...
	                      Py_FileSystemDefaultEncoding, &path2))
		return NULL;
	res = Qp0lRenameUnlink(utfToStr(path1, e_buf), utfToStr(path2, e_buf2));
	PyMem_Free(path1);
	PyMem_Free(path2);
	if (res != 0)
		return posix_error();
...
#define UTIME_ARG buf
    ...
    if (arg == Py_None) {
        ...
        Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
		res = utime(utfToStr(path, e_buf), NULL);
#else
        res = utime(path, NULL);
#endif
        Py_END_ALLOW_THREADS
        ...
        Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
		res = utime(utfToStr(path, e_buf), UTIME_ARG);
#else
...
"spawn(path, args, [env, fdin, fdout, fderr, fdmax])\n\
Starts a child process, returns child pid.\n\
\n\
    path: path of executable file\n\
    args: tuple or list of strings\n\
    env: dict with key and value\n\
    fdin: stdin\n\
    fdout: stdout\n\
    fderr: stderr\n\
    fdmax: Maximum number of inherited file descriptors";

static PyObject *
posix_spawn(PyObject *self, PyObject *args, PyObject *keywds)
{
    char *path, *arg;
    PyObject *argv, *env = NULL;
    static char *kwlist[] = {"path","args","env", "fdin", "fdout", "fderr", "fdmax", NULL};
    char **spawn_argv;
    char **spawn_env;
    char e_path[256];
    struct inheritance  inherit;
    int i, argc, pid, envcount;
    int fdin = -1;
    int fdout = -1;
    int fderr = -1;
    int fdmax = -1;
    int fdcount = 0;
    int fdmap[3];
    PyObject *key, *val, *keys, *vals;
    Py_ssize_t pos;
    char *p, *k, *v, *p2;
    size_t len;

    PyObject *(*getitem)(PyObject *, int);
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "sO|Oiiii:spawn", kwlist, 
                                      &path, &argv, &env, &fdin, &fdout, &fderr, &fdmax))
        return NULL;
    ...
    if (env != NULL && env != Py_None && !PyMapping_Check(env)) {
        PyErr_SetString(PyExc_TypeError, "env must be a mapping object");
        return NULL;
    }
    spawn_argv = PyMem_Malloc(sizeof(char *) * (argc + 2));
    if (spawn_argv == NULL)
        return NULL;
    for (i = 0; i < argc; i++) {
        if (!PyArg_Parse((*getitem)(argv, i), "s", &arg)) {
            PyMem_Free(spawn_argv);
            PyErr_SetString(PyExc_TypeError, 
                    "all arguments must be strings");
            return NULL;        
        } else {
            /* convert to ebcdic */
            spawn_argv[i + 1] = PyMem_Malloc(strlen(arg) + 1);
            utfToStr(arg, spawn_argv[i + 1]);
        }
    }
    ...
    inherit.flags |= SPAWN_SETTHREAD_NP | SPAWN_SETJOBNAMEPARENT_NP;
    /* Environment variables */
    if (env == NULL || env == Py_None) {
        envcount = 0;
        while (environ[envcount] != NULL) { ++envcount; }
        spawn_env = (char **)PyMem_Malloc((envcount + 2) * sizeof(char *));
        for (i = 0; i < envcount; i++)
            spawn_env[i] = environ[i];
    } else {
        keys = PyMapping_Keys(env);
        vals = PyMapping_Values(env);
        envcount = PyList_Size(keys);
        spawn_env = (char **)PyMem_Malloc((envcount + 2) * sizeof(char *));
        if (envcount > 0) {
            for (pos = 0; pos < envcount; pos++) {
                key = PyList_GetItem(keys, pos);
                val = PyList_GetItem(vals, pos);
                if (!PyArg_Parse(key, "s;spawn() env contains a non-string key", &k) ||
                        !PyArg_Parse(val, "s;spawn() env contains a non-string value", &v))
                    return NULL;
                len = PyString_Size(key) + PyString_Size(val) + 2;
                p = PyMem_New(char , len);
                p2 = PyMem_New(char , len);
    ...
    spawn_env[envcount] = NULL;
    /* convert program name to ebcdic */
    utfToStr(path, e_path);
    spawn_argv[0] = e_path;
    /* spawn without and with fdmap */
    if (fdin == -1 && fdout == -1 && fderr == -1) {
        fdcount = sysconf(_SC_OPEN_MAX);
        if (fdmax >= 0 && fdmax < fdcount)
            fdcount = fdmax;
#pragma datamodel(P128)
        pid = spawn(e_path, fdcount, NULL, &inherit, spawn_argv, spawn_env);
#pragma datamodel(pop)
    } else {
        fdmap[0] = fdin;
        fdmap[1] = fdout;
        ...
        if (fdmax > fdcount)
            fdcount = fdmax;
#pragma convert(37)
        spawn_env[envcount] = "QIBM_USE_DESCRIPTOR_STDIO=Y";
#pragma convert(0)
        spawn_env[envcount + 1] = NULL;
#pragma datamodel(P128)
        pid = spawn(e_path, fdcount, fdmap, &inherit, spawn_argv, spawn_env);
#pragma datamodel(pop)
    }
    /* Free memory used for environment parameter */
    if (env != NULL) {
        for (i = 1; i < envcount; i++)
            PyMem_Free(spawn_env[i]);
    }
    for (i = 1; i < argc + 1; i++)
        PyMem_Free(spawn_argv[i]);
    PyMem_Free(spawn_argv);
    PyMem_Free(spawn_env);
    if (pid == -1)
        return posix_error();
...
posix_readlink(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	n = readlink(utfToStr(path, e_buf), buf, (int) sizeof buf);
#else
...
posix_open(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	fd = open(utfToStr(file, e_buf), flag, mode);
#else
...
#ifdef __ILEC400__
	if (putenv(utfToStr(newenv, e_buf))) {
#else
    if (putenv(newenv)) {
#endif
        Py_DECREF(newstr);
...
posix_statvfs(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
#ifdef __ILEC400__
	res = statvfs(utfToStr(path, e_buf), &st);
#else
	res = statvfs(path, &st);
#endif
    Py_END_ALLOW_THREADS
...
posix_pathconf(PyObject *self, PyObject *args)
{
    ...
    errno = 0;
#ifdef __ILEC400__
    limit = pathconf(utfToStr(path, e_buf), name);
#else
...
#ifdef __ILEC400__
PyDoc_STRVAR(iseries_urandom__doc__,
"urandom(n) -> str\n\n\
Return a string of n random bytes suitable for cryptographic use.");

static PyObject*
iseries_urandom(PyObject *self, PyObject *args)
{
...
#pragma convert(37) 
    Qc3GenPRNs(
            (unsigned char*) PyString_AS_STRING(result),
//          crypt,
            howMany,'0','0',&errCode);
#pragma convert(0)  
#pragma datamodel(pop) 
...
//      return;
        Py_DECREF(result);
            return PyErr_Format(PyExc_ValueError,
                                "Iseries Qc3GenPRNs error");

    }
//  result =PyString_FromString(crypt);
    }
    return result;
}
#endif
...
static PyMethodDef posix_methods[] = {
    ...
    {"spawn",           (PyCFunction)posix_spawn, METH_VARARGS|METH_KEYWORDS,
                         posix_spawn__doc__},
#endif
#ifdef HAVE_SPAWNV
 ...
 #endif
 #ifdef __ILEC400__
    {"urandom", iseries_urandom, METH_VARARGS, iseries_urandom__doc__},
 #endif
#ifdef HAVE_SETRESUID
