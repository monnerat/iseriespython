...
#ifdef PG_CHANGES
/* Return new date from a iso string (2003-12-31) or int (20031231) */
static PyObject *
date_fromiso(PyObject *cls, PyObject *args)
{
	PyObject *o, *result = NULL;

	if (PyArg_ParseTuple(args, "O:fromiso", &o)) {
        ...
        if (PyString_Check(o)) {
            ...
            if (s[4] != '-' || s[7] != '-')
                PyErr_SetString(PyExc_ValueError, "string is not iso");
            else
                result = PyObject_CallFunction(cls, "iii", atoi(s),
                        atoi(s+5), atoi(s+8)); 
        } else if (PyInt_Check(o)) {
            int i = PyInt_AS_LONG(o);
            result = PyObject_CallFunction(cls, "iii", i/10000,
                    (i%10000)/100, i%100); 
        } else
            PyErr_SetString(PyExc_TypeError, "fromiso requires a string or int.");
        Py_DECREF(o);
	}
	return result;
}
...
#ifdef PG_CHANGES
static PyObject *
date_isoint(PyDateTime_Date *self)
{
	return PyInt_FromLong(GET_YEAR(self)*10000 + GET_MONTH(self)*100 + GET_DAY(self));
}
#endif
...
#ifdef PG_CHANGES
	{"fromiso", (PyCFunction)date_fromiso,	METH_VARARGS |
							METH_CLASS,
	 PyDoc_STR("str/int -> date from iso formatted string or integer.")},
#endif
...
#ifdef PG_CHANGES
	{"isoint",   (PyCFunction)date_isoint,	METH_NOARGS,
	 PyDoc_STR("Return string as ISO integer, yyyymmdd.")},
#endif
...
#ifdef PG_CHANGES
/* Return new time from an iso string (12:15:00) or a float(121500.3) */
static PyObject *
time_fromiso(PyObject *cls, PyObject *args)
{
	PyObject *o, *result = NULL;
	...
	if (PyArg_ParseTuple(args, "O:fromiso", &o)) {
        ...
        if (PyString_Check(o)) {
            ...
            if (strlen(s) < 8 || s[2] != ':' || s[5] != ':')
                ...
                double d = atof(s+6);
                ms = modf(d, &sec);
                result = PyObject_CallFunction(cls, "iiii", atoi(s),
                        atoi(s+3), (int)sec, (int)(ms * 100000)); 
            }
        } else if (PyInt_Check(o)) {
            int i = PyInt_AS_LONG(o);
            result = PyObject_CallFunction(cls, "iii", i/10000, (i%10000)/100, i%100); 
        } else if (PyFloat_Check(o)) {
            int i, msec;
            double d = PyFloat_AS_DOUBLE(o);
            i = (int)d;
            msec = (int)fmod(d, 1) * 100000;
            result = PyObject_CallFunction(cls, "iiii", i/10000,
                    (i%10000)/100, i%100, msec);
        } else
            PyErr_SetString(PyExc_TypeError, "fromiso requires a string or long int.");
        Py_DECREF(o);
	}
	return result;
}
...
#ifdef PG_CHANGES
static PyObject *
time_isofloat(PyDateTime_DateTime *self, PyObject *unused)
{
    return PyFloat_FromDouble(TIME_GET_HOUR(self)*10000 
            + TIME_GET_MINUTE(self)*100 + TIME_GET_SECOND(self));
}
#endif
/*
 * Miscellaneous methods.
...
static PyMethodDef time_methods[] = {
    ...
    {"isoformat",   (PyCFunction)time_isoformat,        METH_NOARGS,
     PyDoc_STR("Return string in ISO 8601 format, HH:MM:SS[.mmmmmm]"
               "[+HH:MM].")},
#ifdef PG_CHANGES
	{"fromiso", (PyCFunction)time_fromiso,	METH_VARARGS |
							METH_CLASS,
	 PyDoc_STR("str/float -> time from iso formatted string or float.")},
	{"isofloat",	(PyCFunction)time_isofloat,	METH_NOARGS,
	 PyDoc_STR("Return time as a float.")},
#endif
...
#ifdef PG_CHANGES
/* Return new datetime from an iso string (2003-12-31 12:15:00) or number (20031231121500) */
static PyObject *
datetime_fromiso(PyObject *cls, PyObject *args)
{
	PyObject *o, *result = NULL;
        ...
        if (PyString_Check(o)) {
            ...
            if (b < e) {
                if (strlen(s) < 19 || s[4] != '-' || s[7] != '-' ||
                    ...
                    double d = atof(s+17);
                    ms = modf(d, &sec);
                    result = PyObject_CallFunction(cls, "iiiiiii", atoi(s),
                            atoi(s+5), atoi(s+8), atoi(s+11), atoi(s+14),
                            (int)sec, (int)(ms * 100000)); 
                }
            } else {
                /* only digits */
                if (strlen(s) < 14)
                    ...
                    double v = atof(s); 
                    d = (int)(v / 1000000);
                    t = (int)fmod(v, 1000000);
                    result = PyObject_CallFunction(cls, "iiiiii", d/10000,
                            (d%10000)/100, d%100, t/10000, (t%10000)/100, t%100); 
                }
            }
            ...
            PyErr_SetString(PyExc_TypeError, "fromiso requires a string or number.");
        Py_DECREF(o);
	}
	return result;
}
...
#ifdef PG_CHANGES
static PyObject *
datetime_isoint(PyDateTime_DateTime *self, PyObject *unused)
{
    return PyLong_FromDouble((double)(GET_YEAR(self)*10000  
            + GET_MONTH(self)*100) * 1000000
            + GET_DAY(self)*1000000 + DATE_GET_HOUR(self)*10000 
            + DATE_GET_MINUTE(self)*100 + DATE_GET_SECOND(self));
}
#endif
...
#ifdef PG_CHANGES
	{"fromiso", (PyCFunction)datetime_fromiso,	METH_VARARGS |
							METH_CLASS,
	 PyDoc_STR("str/int -> datetime from iso formatted string or integer.")},
#endif
    /* Instance methods: */
...
#ifdef PG_CHANGES
	{"isoint",	(PyCFunction)datetime_isoint,	METH_NOARGS,
	 PyDoc_STR("Return datetime as an int.")},
#endif
