...
open_exclusive(char *filename, mode_t mode)
{
    ...
    */
    int fd;
#ifdef __ILEC400__
    filename = utfToStr(filename, e_buf);
#endif
...
write_compiled_module(PyCodeObject *co, char *cpathname, struct stat *srcstat)
{
    ...
    if (fflush(fp) != 0 || ferror(fp)) {
        ...
        fclose(fp);
#ifdef __ILEC400__
		(void) unlink(utfToStr(cpathname, e_buf));
#else
            ...
            size_t buflen, FILE **p_fp, PyObject **p_loader)
{
    ...
    if (path == NULL) {
        ...
        actmark = getSrvpgm(name, Py_GetProgramLib());
        if (actmark > 0) {
            strcpy(buf, name);
...
#ifdef HAVE_STAT
#ifdef __ILEC400__
		if (stat(utfToStr(buf, e_buf), &statbuf) == 0 && /* it exists */
...
#elif defined(__ILEC400__) || (defined(__MACH__) && defined(__APPLE__) || defined(__CYGWIN__)) && defined(HAVE_DIRENT_H)
    DIR *dirp;
    struct dirent *dp;
...
#ifdef __ILEC400__
	dirp = opendir(utfToStr(dirname, e_buf));
#else    
	dirp = opendir(dirname);
#endif
    if (dirp) {
#ifdef __ILEC400__
        char *nameWithExt = utfToStr(buf + len - namelen, e_buf);
#else
...
#ifdef __ILEC400__
	if (stat(utfToStr(buf, e_buf), &statbuf) == 0) {
#else
...
#ifdef __ILEC400__
		rv = stat(utfToStr(path, e_buf), &statbuf);
#else
